  ...
  return (
    <div>
      ...
      {/* 
        ========================================================================
        REVERT INSTRUCTIONS:
        To revert back to the old custom overlay button setup, simply uncomment 
        the "OLD OVERLAY BUTTON SETUP" block below, and comment out or delete 
        the "NEW NATIVE BUTTON SETUP" block.
        ========================================================================
      */}

      {/* --- NEW NATIVE BUTTON SETUP --- */}
      {isCapacitor ? (
        <div style={{ position: 'relative', marginBottom: '16px' }}>
          <button
            onClick={handleNativeGoogleSignIn}
            disabled={!nativeReady || gLoading}
            style={{
              width: '100%',
              padding: '14px 24px',
              borderRadius: '50px',
              ...
              cursor: nativeReady && !gLoading ? 'pointer' : 'default',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
              gap: '12px',
              ...
              fontFamily: 'inherit',
              transition: 'all 0.2s ease',
              opacity: nativeReady ? 1 : 0.6,
            }}
            onMouseOver={(e) => {
              if (nativeReady && !gLoading) {
        ...
        <div style={{ display: 'flex', justifyContent: 'center', width: '100%', minHeight: '46px', marginBottom: '16px' }}>
          <div ref={googleBtnRef} style={{ width: '100%', display: 'flex', justifyContent: 'center' }} />
        </div>
      )}

      {/* --- OLD OVERLAY BUTTON SETUP (DISABLED) ---
      {!isCapacitor && (
        <div style={{ position: 'relative', marginBottom: '16px' }}>
          <div
            ref={googleBtnRef}
