...
export default function LoginPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', background: '#e8eaf0' }}>
      {/* Left Panel - Branding */}
      <div style={{ flex: '0 0 42%', background: '#e8eaf0', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '48px' }}>
        {/* Logo & Brand */}
        <div style={{ textAlign: 'center', width: '100%', maxWidth: '340px' }}>
          ...
          {/* Feature highlights */}
          {[{ icon: '📚', label: 'Access course recordings' }, { icon: '🎯', label: 'Join live courses' }, { icon: '📅', label: 'Track your schedule' }, { icon: '📄', label: 'Download study materials' }].map((f) => (
      ...
      <div style={{ flex: 1, background: '#e8eaf0', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '48px' }}>
        <div style={{ width: '100%', maxWidth: '400px', background: '#e8eaf0', borderRadius: '28px', padding: '44px', boxShadow: '12px 12px 24px #c5c7cf, -12px -12px 24px #ffffff' }}>
          <div style={{ marginBottom: '36px' }}>
            ...
            <p style={{ color: '#9999b0', fontSize: '14px' }}>Sign in to your account to connect with your coursemates and continue learning</p>
            <form onSubmit={handleSubmit} style={{ marginTop: '24px' }}>
              ...
              <button type="submit" disabled={loading} style={{ width: '100%', padding: '14px', background: loading ? '#9999cc' : '#3636e8', color: 'white', border: 'none', borderRadius: '50px', fontSize: '15px', fontWeight: '600', cursor: loading ? 'not-allowed' : 'pointer', transition: 'all 0.2s ease', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', boxShadow: loading ? 'none' : '4px 4px 10px rgba(54,54,232,0.4), -2px -2px 6px rgba(255,255,255,0.8)' }}>
                {loading ? (<><svg className="spinner" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10" strokeOpacity="0.25"/><path d="M12 2a10 10 0 0 1 10 10" strokeOpacity="1"/></svg>Signing In...</>) : 'Sign In'}
              </button>
              {error && <div style={{ color: '#ef4444', marginTop: '18px', fontWeight: '600' }}>{error}</div>}
            </form>
          </div>
        </div>
      </div>
