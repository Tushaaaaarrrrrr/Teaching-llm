      ...
      {course.courseEvents && course.courseEvents.length > 0 && (
        <div style={{ marginBottom: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
            <h3 style={{ fontSize: '16px', fontWeight: '700', color: '#1e1e3a', display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {course.courseEvents.slice(0, 3).map(ev => {
              const start = new Date(ev.startTime)
              const isLive = new Date() >= start && new Date() <= new Date(ev.endTime)
              return (
                <div key={ev.id} style={{
                  display: 'flex', alignItems: 'center', gap: '16px',
                  padding: '16px 24px', borderRadius: '50px',
                  background: isLive ? '#fff1f2' : '#e8eaf0',
                  boxShadow: '6px 6px 12px #c5c7cf, -6px -6px 12px #ffffff',
                }}>
                  <div style={{
                    width: '44px', height: '44px', borderRadius: '12px',
                    background: isLive ? '#f43f5e' : '#fff',
                    display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
                  }}>
                    <span style={{ fontSize: '14px', fontWeight: '800', color: isLive ? '#fff' : '#1e1e3a', lineHeight: 1 }}>{start.getDate()}</span>
                    <span style={{ fontSize: '9px', fontWeight: '700', color: isLive ? 'rgba(255,255,255,0.8)' : '#9999b0', textTransform: 'uppercase' }}>
                      {start.toLocaleDateString('en-US', { month: 'short' })}
                    </span>
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '2px' }}>
                      <span style={{ fontSize: '14px', fontWeight: '700', color: '#1e1e3a' }}>{ev.title}</span>
                      {isLive && (
                        <span style={{ background: '#f43f5e', color: 'white', fontSize: '9px', fontWeight: '800', padding: '2px 8px', borderRadius: '10px', textTransform: 'uppercase' }}>LIVE</span>
                      )}
                    </div>
                    <div style={{ fontSize: '12.5px', color: '#6b6b8a' }}>
                      {start.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} &middot; {ev.instructor?.name || course.teacherName || 'TBA'}
                    </div>
                  </div>
                  {ev.meetLink && (
                    <a href={ev.meetLink} target="_blank" rel="noreferrer" className={isLive ? 'btn btn-primary' : 'btn btn-outline'} style={{
                      borderRadius: '50px', padding: '10px 24px', fontSize: '13px', fontWeight: '700',
                      background: isLive ? '#f43f5e' : undefined,
                      borderColor: isLive ? '#f43f5e' : undefined,
                      color: isLive ? '#fff' : undefined,
                    }}>
                      Join Session
                    </a>
                  )}
