...
const STATUS_CONFIG: Record<string, { label: string; color: string; dotColor: string; bg: string }> = {
  ...
  active:      { label: 'ACTIVE',      color: '#3b82f6', dotColor: '#3b82f6', bg: 'rgba(59,130,246,0.10)' },
...
export default function LivePage() {
  ...
  const renderSessionBlock = (session: LiveSession, isNextUpcoming: boolean) => {
    ...
    const dotColor = isLive ? (session.status === 'active' ? '#3b82f6' : '#16a34a') : isCancelled ? '#ef4444' : isRescheduled ? '#d97706' : '#c5c7cf'
    const dotFill = isLive ? (session.status === 'active' ? '#3b82f6' : '#16a34a') : '#e8eaf0'
    const dotGlow = isLive
      ? (session.status === 'active' ? '0 0 0 4px rgba(59,130,246,0.18)' : '0 0 0 4px rgba(22,163,74,0.18)')
      : isNextUpcoming
    ...
    const rowShadow = isCompleted || isCancelled
      ...
      : isLive
      ? (session.status === 'active' ? '6px 6px 14px #b8d4e8, -6px -6px 14px #ffffff' : '6px 6px 14px #b8e8c8, -6px -6px 14px #ffffff')
      : isNextUpcoming
      ...
      <div key={session.id} style={{ position: 'relative', marginBottom: '14px', display: 'flex', alignItems: 'center', gap: '20px' }}>
        ...
        <div style={{
          ...
          <div style={{ width: '120px', flexShrink: 0, overflow: 'hidden' }}>
            <div style={{ fontSize: '13px', fontWeight: '600', color: isLive ? (session.status === 'active' ? '#3b82f6' : '#16a34a') : isCompleted || isCancelled ? '#9999b0' : '#6b6b8a', whiteSpace: 'nowrap' }}>
              {session.time}
            </div>
          ...
          {isCompleted ? (
            ...
            <a href={session.meetingLink} target="_blank" rel="noopener noreferrer" style={{
              ...
              background: session.status === 'active' ? '#3b82f6' : '#16a34a', color: 'white', fontWeight: '600', fontSize: '14px', textDecoration: 'none',
              boxShadow: session.status === 'active' ? '4px 4px 10px rgba(59,130,246,0.4), -2px -2px 6px rgba(255,255,255,0.8)' : '4px 4px 10px rgba(22,163,74,0.4), -2px -2px 6px rgba(255,255,255,0.8)',
            }}>
              Join
