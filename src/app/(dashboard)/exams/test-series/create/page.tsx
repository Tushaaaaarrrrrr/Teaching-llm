...
export default function CreateTestSeriesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('/api/test-series/create-with-exam', {
        method: 'POST',
        body: formData
      })
      ...
      if (res.ok) {
        ...
        alert(err.error || 'Failed to create test series')
      }
    } catch (error) {
      console.error(error)
          ...
          <div style={neuCard}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: requiresSchedule ? '1fr 1fr 1fr' : '1fr', gap: '16px' }}>
                ...
                type="button" 
                onClick={() => {
                  if (!title || !durationMinutes || (requiresSchedule && (!startDate || !expiresAt))) {
        ...
        {step === 2 && (
          <>
            ...
            <button type="submit" disabled={loading} style={{ ...neuButton, width: '100%', padding: '16px', fontSize: '16px', opacity: loading ? 0.7 : 1, marginTop: '12px' }}>
              {loading ? 'Processing...' : 'Finalize Test Series'}
            </button>
          </>
        )}
      </form>
